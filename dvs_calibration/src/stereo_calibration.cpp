#include "dvs_calibration/stereo_calibration.h"
...
StereoCalibration::StereoCalibration()
{
  ...
  startCalibrationService = nh.advertiseService("dvs_calibration/start", &StereoCalibration::startCalibrationCallback, this);
  saveCalibrationService = nh.advertiseService("dvs_calibration/save", &StereoCalibration::saveCalibrationCallback, this);
  resetService = nh.advertiseService("dvs_calibration/reset", &StereoCalibration::resetCallback, this);

  setCameraInfoClient = nh.serviceClient<sensor_msgs::SetCameraInfo>("set_camera_info");
  cameraInfoSubscriber = nh.subscribe("camera_info", 1, &StereoCalibration::cameraInfoCallback, this);

  eventSubscriber = nh.subscribe("events", 10, &StereoCalibration::eventsCallback, this);
  ...
  leftEventSub = nh.subscribe<dvs_msgs::EventArray>("/dvs_left/events", 1,
                                                    boost::bind(&StereoCalibration::eventsCallback2, this, _1, left_camera_id));
  rightEventSub = nh.subscribe<dvs_msgs::EventArray>("/dvs_right/events", 1,
                                                     boost::bind(&StereoCalibration::eventsCallback2, this, _1, right_camera_id));

  // initialize world pattern
...
void StereoCalibration::eventsCallback2(const dvs_msgs::EventArray::ConstPtr& msg, int camera_id)
{
  ROS_INFO("Got events from camera with id %d", camera_id);
  // transition_maps[camera_id].update(camera_id);
  // if (transition_maps[camera_id].max() > threshold) {
...
void StereoCalibration::eventsCallback(const dvs_msgs::EventArray::ConstPtr& msg)
{
...
void StereoCalibration::reset_maps()
{
//  for (int i = 0; i < sensor_width; i++)
//  {
//    for (int j = 0; j < sensor_height; j++)
...
std::vector<cv::Point2f> StereoCalibration::findPattern()
{
...
void StereoCalibration::publishVisualizationImage(std::vector<cv::Point2f> pattern)
{
  cv_bridge::CvImage cv_image;
  cv_image.encoding = "bgr8";
  //cv_image.image = transition_maps[...].get_visualization_image();
...
void StereoCalibration::publishPatternImage(cv::Mat image)
{
  cv_bridge::CvImage cv_image;
...
void StereoCalibration::resetIntrinsicCalibration()
{
...
bool StereoCalibration::startCalibrationCallback(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response)
{
...
bool StereoCalibration::resetCallback(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response)
{
  calibration_running = false;
...
bool StereoCalibration::saveCalibrationCallback(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response)
{
  return setCameraInfo();
}

bool StereoCalibration::setCameraInfo()
{
...
void StereoCalibration::cameraInfoCallback(const sensor_msgs::CameraInfo::ConstPtr& msg)
{
  gotCameraInfo = true;
  cameraInfo = *msg;
}
